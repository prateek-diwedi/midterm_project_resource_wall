...
const increaseRating = function (rating) {
...
const addUserResourceRating = function (userId, resourceId) {
  return pool.query(`
  INSERT INTO ratings (user_id, resource_id) VALUES ($1, $2) RETURNING *;`, [userId, resourceId]);
};
...
const getAllProperties = function (options, limit = 10) {
  ...
  // 2
  let queryString = `
  SELECT learning_resources.*, users.email, ROUND (avg(ratings.rating)) as rating, count(likes.resource_id) as liked
  FROM learning_resources
  LEFT JOIN users ON users.id = user_id
  LEFT JOIN categories ON categories.id = category_id
  LEFT JOIN likes ON resource_id = learning_resources.id
  LEFT JOIN comments ON resource_id = learning_resources.id
  LEFT JOIN ratings ON resource_id = learning_resources.id
  `;
