...
const addResource = function(resource) {
  ...
  INSERT INTO learning_resources (url, url_image, title, description, category_id)
    VALUES ( $1, $2, $3, $4, $5)
    RETURNING *;
  `, [resource.url, resource.url_image, resource.title, resource.description, resource.category_id])
    .then(res => {
      if (res.rows.length) {
...
const getAllProperties = function (options, limit = 10) {
  ...
  // 2
  let queryString = `
  SELECT learning_resources.*, avg(resource_reviews.rating) as average_rating, count(resource_reviews.liked) as liked
